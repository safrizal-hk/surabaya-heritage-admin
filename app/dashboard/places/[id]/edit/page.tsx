...
export default function EditPlacePage({ params }: PageProps) {
  ...
  const handleAddPhotoSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      // If there are no photos currently, make this first photo the primary one
      const isPrimary = photos.length === 0;
      
      addPhoto(placeId, newPhotoUrl.trim(), newPhotoCaption.trim() || "Landmark View", isPrimary);
      setNewPhotoUrl("");
      setNewPhotoCaption("");
  ...
  const handleSetPrimary = (photoId: number) => {
    const success = setPrimaryPhoto(placeId, photoId);
    if (success) {
      toast({
  ...
  return (
    <div className="space-y-6 max-w-full mx-auto">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-zinc-950">Edit Heritage Place</h1>
        </div>
      </div>
                        ...
                        <div className="flex items-center justify-between gap-1 mt-1">
                          {photo.is_primary ? (
                            ...
                            <Button
                              type="button"
                              variant="ghost"
                              className="h-5 px-1.5 text-[9px] font-semibold text-zinc-500 hover:text-zinc-950 shrink-0 cursor-pointer"
