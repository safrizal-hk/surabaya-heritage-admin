...
export default function PlacesTable({
  ...
  const { places: contextPlaces, categories: contextCategories, placePhotos: contextPhotos, deletePlace } = useDB();
  ...
  const handleDeletePlace = async () => {
    if (deleteConfirmId !== null) {
      const success = await deletePlace(deleteConfirmId);
      if (success) {
        toast({
          title: "Place Deleted",
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-between border-t border-zinc-200/80 p-4 bg-zinc-50/50">
              <div className="text-xs text-zinc-500">
                ...
                of <span className="font-semibold text-zinc-950">{totalItems}</span> places
              </div>
